...
// Single canonical contact path for the whole page. Kept here at the top
// so the rest of the file references one constant — no scattered email
// strings to drift over time.
const SUPPORT_EMAIL = "support@standbybooth.com";
const SUPPORT_MAILTO = "mailto:" + SUPPORT_EMAIL + "?subject=Standby%20Booth%20%E2%80%94%20Access%20request";

// Tap-target sized icon button that flips data-theme on <html>, persists
// to localStorage, and keeps the iOS Safari status-bar tint in sync via
// the #sb-theme-color meta. Initial value is set by the inline boot script
// in index.html (before paint) to avoid flash-of-wrong-theme.
const ThemeToggle = () => {
  ...
  const toggle = () => {
    ...
    const meta = document.getElementById("sb-theme-color");
    if (meta) meta.setAttribute("content", next === "dark" ? "#14110D" : "#F5F1E8");
  };
  ...
  return (
    ...
    >
      {/* Both icons rendered; CSS toggles which is visible based on data-theme.
          Keeping both in DOM avoids layout shift on click. */}
...
// In the wild — four real-world product renders. Each PNG has its headline
// + subhead baked into the image, so we surface that text as alt for screen
// readers and use the caption row only for the use-case label. The grid is
// 2×2 on desktop and collapses to a single column under ~768 px.
const IN_THE_WILD_PLATES = [
  {
    src: "assets/hero-anywhere.png",
    alt: "Your studio. Anywhere. Turn any iPad into a video booth.",
    caption: "Plate A · Corporate · Desk"
  },
  {
    src: "assets/use-bachelorette.png",
    alt: "Every guest has a story. Capture video messages in the moment.",
    caption: "Plate B · Bachelorette · Florals"
  },
  {
    src: "assets/use-wedding.png",
    alt: "No crew. No setup. Guests tap, record, and walk away.",
    caption: "Plate C · Wedding · Reception"
  },
  {
    src: "assets/use-home.png",
    alt: "The screen is the studio. White margins glow like soft fill light.",
    caption: "Plate D · At home · Soft light"
  }
];

const InTheWild = () => (
  <section className="in-the-wild" id="wild">
    <p className="feature-no">Interlude · In the wild</p>
    <h2 className="display wild-hd">Four rooms. <em>One screen.</em></h2>
    <p className="wild-deck">The same Standby Booth attract screen, dropped into four very different evenings. White margins do the lighting; the iPad does everything else.</p>
    <div className="wild-grid">
      {IN_THE_WILD_PLATES.map((p, i) => (
        <figure className="wild-plate" key={i}>
          <img src={p.src} alt={p.alt} loading="lazy" />
          <figcaption className="mono dim">{p.caption}</figcaption>
        </figure>
      ))}
    </div>
  </section>
...
const SignOff = () => {
  ...
  return (
    <section className="signoff" id="access">
      <h2 className="signoff-h">Tell us about your <em>event.</em></h2>
      <p className="signoff-p">Setting up a booth for a wedding, a brand activation, or a launch party? Send us a note. We read every reply.</p>
      <div className="signoff-cta">
        <a className="btn btn-primary" href={SUPPORT_MAILTO}>Email us</a>
        <button className="btn btn-ghost" onClick={async () => {
          try { await navigator.clipboard.writeText(SUPPORT_EMAIL); setCopied(true); setTimeout(()=>setCopied(false), 1800); } catch(e){}
        }}>{copied ? "Copied ✓" : "Copy address"}</button>
      </div>
      <p style={{marginTop:24, fontFamily:"var(--mono)", fontSize:11, letterSpacing:"0.12em", textTransform:"uppercase", color:"var(--ink-2)"}}>{SUPPORT_EMAIL}</p>
    </section>
  );
};
